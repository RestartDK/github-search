...
describe("useSearchProfiles", () => {
	beforeEach(() => {
		// Setup fetch mock
		vi.stubEnv("VITE_GITHUB_URL", "https://api.github.com/");
	...
	beforeEach(() => {
		...
		vi.spyOn(global, "fetch").mockImplementation(
			(url: string | URL | Request) => {
				...
				if (
					urlString.includes("/users/testuser") &&
					!urlString.includes("/repos") &&
				...
				// Mock PRs endpoint
					...
					urlString.includes("search/issues") &&
					urlString.includes("type:pr")
				) {
					return Promise.resolve({
						ok: true,
						json: () =>
